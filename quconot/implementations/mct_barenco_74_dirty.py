...
class MCTBarenco74Dirty(MCTBase):
    ...
    **Multi-controlled not implementation using Corollary 7.4 of Barenco et al:**
    ...
    On an :math:`n`-bit network (where :math:`n ≥ 7`), a :math:`∧_{n-2}(\sigma_{x})` gate can be simulated by ``8(n−5)`` :math:`∧_{2}(\sigma_{x})` gates (3-bit Toffoli gates), as well as by ``48n−204`` basic operations.
    In this implementation, at least one auxiliary qubit is required i.e. in an :math:`n` qubit network is required for an :math:`(n-1)` qubit gate :math:`∧_{n-2}(\sigma_{x})`

    First, Lemma 7.2 is applied, with :math:`m_{1} = \lceil \frac{n}{2} \rceil` and :math:`m_{2} = n - m_{1} - 1` to simulate :math:`∧_{m_1}(\sigma_{x})` and :math:`∧_{m_2}(\sigma_{x})` gates.
    It allows a :math:`∧_{m}(\sigma_{x})` gate to be simulated by a network consisting of ``4(m-2)`` :math:`∧_{2}(\sigma_{x})` gates and is of the form:

    Using Lemma 7.3, these gates are combined to simulate the :math:`∧_{n-2}(\sigma_{x})` gate.
    It allows a :math:`∧_{n-2}(\sigma_{x})` gate to be simulated by a network of two :math:`∧_{m}(\sigma_{x})` and two :math:`∧_{n-m-1}(\sigma_{x})` gates.

    Almost all of the Toffoli gates need only to be simulated modulo phase factors, except 4 Toffoli gates which involve the last bit which need to be simulated without it. Thus, these 4 gates are simulated by 16 basic operations, while the other ``8n-36`` Toffoli gates are simulated in just 6 basic operations.
    """
