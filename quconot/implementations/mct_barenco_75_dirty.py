...
class MCTBarenco75Dirty(MCTBase):
    ...
    **Multi-controlled not implementation using Corollary 7.5 of Barenco et al:**
        ...
        **Proof:**
        Let :math:`V` be such that :math:`V^2 = U`.
        If the any of the first ``n-1`` qubits are ``0`` then the transformation applied to the last qubit is either :math:`I` or :math:`V · V^† = I`.
        If the first ``n-1`` qubits are all ``1`` then the transformation applied to the last qubit is :math:`V · V = U`
