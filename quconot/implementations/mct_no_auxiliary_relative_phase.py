...
class MCTNoAuxiliaryRelativePhase(MCTBase):
    ...
    r"""
    This is an implementation of the simplified Toffoli gates according to Qiskit. Currently, for this implementation, the number of controls is restricted to 2 or 3 only. They have been implemented using the RCCXGate and the RC3XGate in Qiskit.
    It implements the 2 or 3 controlled Toffoli gate up to relative phases.

    The simplified Toffoli is not equivalent to the Toffoli, but can be used in places where the Toffoli is uncomputed again.
