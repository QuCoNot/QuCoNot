...
class MCTRecursion(MCTBase):
    r"""
    Implementation of the multi-controlled not gate with no auxiliary qubits using Qiskit’s ``recursion`` mode.
    This is implemented using MCXRecursive.
    Using one auxiliary qubit, the MCT gate is recursively split onto four sub-registers.
    This is done until the 3- or 4-controlled X gate is reached, since for these we have a concrete implementation that do not require ancillas.
    """
